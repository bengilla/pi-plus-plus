...
interface Props {
  ...
  standaloneConversations: ConvInfo[];
  activeConvId: string | null;
  onNewConversation: () => void;
  onNewStandaloneConversation: () => void;
...
export function Sidebar({
  ...
  // Shared conversation item renderer (used by both standalone + project lists)
        ...
        {expandedChats && (
          <div className="flex-1 overflow-y-auto px-2 pb-2 min-h-0">
            {standaloneConversations.length === 0 ? (
              ...
              <div className="pl-3 pr-1">
                {standaloneConversations.map((c) => renderConvItem(c, true))}
              </div>
            )}
          </div>
        )}
